#!/usr/bin/env python
"""
GP extrabol pipeline. Hot paths use :mod:`yse.extrabol._extrabol_accel` (vectorized
NumPy + optional Numba). Install ``pip install -e ".[extrabol-gp]"`` for ``synphot``
and ``numba``.
"""
...
import glob
import copy
# Nebular synthetic photometry (synphot; https://synphot.readthedocs.io/ )
from yse.extrabol._synphot_phot import (
    observation_effective_magnitude,
    source_spectrum_flam,
    spectral_element_throughput,
)

from yse.extrabol._extrabol_accel import (
    bbody_numpy,
    build_filter_index_map,
    chi_square_vec,
    get_flam_batch,
)
...
def bbody(lam, T, R):
    ...
    ----------
    lam : float or array
        Reference wavelength(s) in Angstroms
    T : float
    ...
    Output
    ------
    L_lam in erg/s/cm (scalar or array matching ``lam``)
    '''
    return bbody_numpy(lam, T, R)

def import_nebular(
    bandpasses,
    filter_names,
    use_jerkstrand=False,
    use_dessart=True,
    nebular_normalize=1.0e41,
    unit="AB",
    nebular_dir="../nebular",
):
    """Load Dessart nebular models and compute synthetic mags per ``filter_names``.

    ``bandpasses`` and ``filter_names`` must be the same length (same order as SVO).
    """
    ...
    if use_dessart:
        for file in sorted(glob.glob(os.path.join(d_dir, '*.fl'))):
            ...
            sp = source_spectrum_flam(wave, flux)

            data = {
                "file": file,
                "normalize": nebular_normalize,
                "unit": unit,
                "teff": 2500.0,
            }

            for filt_name, bp in zip(np.asarray(filter_names).ravel(), bandpasses):
                mag = observation_effective_magnitude(sp, bp, wave, unit)
                data[str(filt_name)] = mag
...
def read_in_photometry(filename, dm, redshift, start, end, snr, mwebv,
    ...
    # Extract filter names and effective wavelengths (O(1) lookup per row)
    idx_map = build_filter_index_map(filterIDs)
    try:
        row_ids = np.array([idx_map[str(uf)] for uf in photometry_data[:, 3]], dtype=np.intp)
    except KeyError as e:
        sys.exit(f"Cannot find filter {e!s} in SVO.")
    wv_effs = wavelengthEffs[row_ids]
    width_effs = widthEffs[row_ids]
    my_filters = photometry_data[:, 3].copy()

    # Convert brightness data to flux (vectorized)
    mags = photometry_data[:, 1].astype(float) - dm
    is_ab = photometry_data[:, -1] == "AB"
    zpt_row = np.asarray(zpts_all[row_ids], dtype=float)
    zpts_use = np.where(is_ab, 3631.00, zpt_row)
    flux_lin = 10.0 ** (mags / -2.5) * zpts_use / (1.0 + redshift)
    fluxes = 2.5 * (np.log10(flux_lin) - np.log10(3631.00))

    # Remove extinction
    if use_wc:
        ext = extinction.fm07(wv_effs / (1.0 + redshift), mwebv)
    else:
...
def chi_square(dat, model, uncertainty):
    ...
    '''
    return chi_square_vec(np.asarray(dat), np.asarray(model), np.asarray(uncertainty))
...
def generate_template(filter_wv, sn_type):
    ...
    # The template is too large, so we thin it out (vectorized masks)
    wmin, wmax = np.amin(filter_wv), np.amax(filter_wv)
    gis = (temp_wavelength > wmin) & (temp_wavelength < wmax)
    temp_times = temp_times[gis]
    temp_wavelength = temp_wavelength[gis]
    temp_f_lambda = temp_f_lambda[gis]

    gis = (temp_times % 2.0) == 0.0
    temp_times = temp_times[gis]
    temp_wavelength = temp_wavelength[gis]
    temp_f_lambda = temp_f_lambda[gis]

    gis = (temp_wavelength % 20.0) == 0.0
    temp_times = temp_times[gis]
    temp_wavelength = temp_wavelength[gis]
    temp_f_lambda = temp_f_lambda[gis]
...
def interpolate(lc, wv_corr, sn_type, use_mean, z, verbose, filter_mean_function, 
    ...
    if not use_mean:
        ...
        for i, filt in enumerate(cubic_filters):
            ...
            kernel = np.var(y) * george.kernels.Matern32Kernel(1e6)
            cubic_spline = CubicSpline(sorted_time, sorted_mag)
            class snModel_cubic:
        ...
        for i, filt in enumerate(template_filters): 
            ...
            class snModel(Model):
                def get_value(self, param):
                    t = (param[:, 0] * 1.0 / t_stretch) + t_shift
                    wv = param[:, 1]
                    return np.asarray(template.ev(t, wv) + f_stretch, dtype=np.float64).ravel()
...
def get_flam(datapoint):
    ...
    """
    fl, fle = get_flam_batch(
        np.asarray([datapoint[0]], dtype=float),
        np.asarray([datapoint[1]], dtype=float),
        np.asarray([datapoint[2]], dtype=float),
    )
    return float(fl[0]), float(fle[0])
...
def fit_bb(epoch_data, use_mcmc=False, T_max=40000., save_chains=False,
    ...
    for i, epoch in enumerate(epoch_data):
        ...
        stack = np.array([[e[0], e[1], e[2]] for e in epoch[1:]], dtype=float)
        wavelengths = stack[:, 2]
        flams, flamerrs = get_flam_batch(stack[:, 0], stack[:, 1], wavelengths)
...
def fit_nebular(epoch_data, filters, wvs, use_mcmc=False, delta_time=1.0):
    ...
    for filt in filters:
        ...
        tc_col = "TrasmissionCurve"
        if tc_col not in index.colnames:
            tc_col = "TransmissionCurve"
        transdata = Table.read(index[mask][tc_col][0])
        ...
        bp = spectral_element_throughput(
            transdata["Wavelength"].data,
            transdata["Transmission"].data,
            name=str(filt),
        )
        bps.append(bp)

    nebular_models = import_nebular(bps, filters)
...
def plot_bb_ev(epoch_data, Tarr, Rarr, Terr_arr, Rerr_arr, snname, sn_type,
    ...
    len_Tarr = len(Tarr)
    if nTarr is None:
        nTarr = np.array([], dtype=float)
    if nRarr is None:
        nRarr = np.array([], dtype=float)
    len_nTarr = len(nTarr)
